...
@DisplayName("SQLite DPM DB content: configurable optional features")
internal class DpmDbWriter_ContentOptionals_UnitTest : DpmDbWriter_ContentUnitTestBase() {
    ...
    inner class MandatoryLabelTranslationOption {
        ...
        @Test
        fun `should support configurable target and source languages (FI injected to EN)`() {
            setupDbViaDictionaryCreate(
                false,
                FixtureVariety.TRANSLATIONS_FI_ONLY,
                mapOf(
                    DpmModelOptions.SqliteDb_MandatoryLabelTranslation_TargetLanguage to
                        Language.byIso6391CodeOrFail("en"),

                    DpmModelOptions.SqliteDb_MandatoryLabelTranslation_SourceLanguages to
                        listOf(
        ...
        @Test
        fun `should treat source languages as prioritized list (SV injected to EN)`() {
            setupDbViaDictionaryCreate(
                false,
        ...
        @Test
        fun `should not produce label translation when no suitable source translation is found`() {
            setupDbViaDictionaryCreate(
                false,
                FixtureVariety.TRANSLATIONS_FI_ONLY,
                mapOf(
    ...
    inner class DpmElementUriStorageOption {
        ...
        @Test
        fun `should support configurable target language (URI stored as PL)`() {
            setupDbViaDictionaryCreate(
                false,
                FixtureVariety.NONE,
                mapOf(
        ...
        @Test
        fun `should produce warning when target language already has a translation (URI stored as FI)`() {
            setupDbViaDictionaryCreate(
                false,
                FixtureVariety.NONE,
                mapOf(
